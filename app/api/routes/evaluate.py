...
--------------------------
Evaluation endpoints backed by SQLite query logs, scoped to the caller.

Endpoints:
    GET  /api/v1/queries              → list recent logged queries (own only)
    GET  /api/v1/queries/{query_id}   → get a single logged query (own only)
    POST /api/v1/evaluate/{query_id}  → run RAGAS on one query (own only)
    POST /api/v1/evaluate/all         → run RAGAS on last N queries (own only)
"""
...
def list_queries(
    ...
    """List the most recent RAG query logs belonging to the caller."""
    rows = get_recent_queries(user_id=user_id, limit=limit)
...
def get_single_query(
    ...
    """Fetch a single query log by query_id — only if it belongs to the caller."""
    row = get_query(query_id, user_id=user_id)
    if not row:
...
def evaluate_single(
    ...
    """
    Run RAGAS evaluation on a single logged query belonging to the caller.
...
def evaluate_all(
    ...
    """
    Run RAGAS evaluation on the caller's last N logged queries.
